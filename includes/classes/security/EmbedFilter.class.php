...
if (!class_exists('\\X2board\\Includes\\Classes\\Security\\\EmbedFilter')) {
	...
	class EmbedFilter {
		...
		function __construct() {
			$s_cached_filter_path = wp_get_upload_dir()['basedir'].DIRECTORY_SEPARATOR.X2B_CACHE_PATH.DIRECTORY_SEPARATOR.'embedfilter';
			if( !file_exists( $s_cached_filter_path ) ) {
				wp_mkdir_p( $s_cached_filter_path );
			}
		...
		private function _makeWhiteDomainList($whitelist = NULL) {
			...
			if($isMake)	{
				...
				if( (isset($whitelist->object) && gettype($whitelist->object) == 'array') && 
					( isset($whitelist->object) && gettype($whitelist->iframe) == 'array') ) {
				...
				$buff = array();
				$buff[] = '<?php if(!defined("ABSPATH")) exit();';
