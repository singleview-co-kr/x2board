	...
	class Context {
		...
		public static function get_url( $num_args = 0, $args_list = array(), $domain = null, $encode = true, $autoEncode = false ) {
			...
			if ( ! $o_self->get_vars || $args_list[0] == '' ) {
				...
				if ( (bool) $o_self->get_vars->cmd ) {
					$get_vars['cmd'] = $o_self->get_vars->cmd;
				}
				if ( (bool) $o_self->get_vars->page ) {
					$get_vars['page'] = $o_self->get_vars->page;
				}
				if ( (bool) $o_self->get_vars->search_target ) {
					$get_vars['search_target'] = $o_self->get_vars->search_target;
				}
				if ( (bool) $o_self->get_vars->search_keyword ) {
					$get_vars['search_keyword'] = $o_self->get_vars->search_keyword;
				}
			}
