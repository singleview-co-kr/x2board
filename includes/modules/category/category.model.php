	...
	class categoryModel extends category {
		...
		private function _get_old_category() {
			...
			$results = $wpdb->get_results(
				'SELECT `category_id`, `board_id`, `title`, `group_ids`, `parent_id`, `expand`, `color`, `is_default`, `post_count` ' .
			...
			foreach ( $results as $row ) {
				...
				$a_category[ $n_category_id ] = array(
					'id'         => $n_category_id,
			...
			foreach ( $list as $key => $val ) {
				...
				if ( $obj->parent_id ) {
					...
					while ( $parent_id ) {
						...
						if ( $expand ) {
							$document_category[ $parent_id ]->expand = $expand;
						}
				...
				if ( isset( $val['children'] ) && count( $val['children'] ) ) {
					$this->_arrange_category( $document_category, $val['children'], $depth + 1 );
				}
			}
