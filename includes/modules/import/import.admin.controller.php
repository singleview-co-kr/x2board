...
if (!class_exists('\\X2board\\Includes\\Modules\\Import\\importAdminController')) {
	...
	class importAdminController {
		...
		private $_a_image_ext = array('jpg|jpeg|jpe'=>'image/jpeg', 'png'=>'image/png', 'gif'=>'image/gif');
		...
		public function import_xe_xml($s_uploaded_file, $s_uploaded_filename){
			...
			// $o_mapper->new_base_url = null;
			$o_mapper->map = [];

			// $wp_page_id = $wpdb->get_var("SELECT `value` FROM `{$wpdb->prefix}kboard_board_meta` WHERE `key`='auto_page' AND `board_id`='$this->_n_board_id'");
			$post = get_post( intval($this->_n_board_id) );
			// $wpdb->flush();

			// $o_mapper->new_base_url = $post->post_title.'?document'; // x2board pretty url format
			...
			// var_dump(wp_json_file_decode($s_base_path.DIRECTORY_SEPARATOR.'mapper.json'));
	// exit;
		}
...
// var_dump($a_wp_posts_to_delete_comment);
				...
				foreach($array['posts']['categories']['category'] as $cat_idx => $cat_rec){
					...
					$a_cat_info[$n_xe_cat_id] = array('xe_cat_name'=>$s_new_cat_name,
...
// var_dump($a_linear_cat_info);				
			...
			$seo_translation_mapper = [];
			$user_translation_mapper = [];  // $user_translation_mapper[<string>xe_user_id] = <int>wp_user_id
...
// var_dump($a_attaches_from_xml);
			foreach( $a_attaches_from_xml as $_ => $single_attach_info) {
// var_dump($source_file_path_abs); 				
