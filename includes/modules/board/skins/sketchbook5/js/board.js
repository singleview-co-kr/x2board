...
function board(bdObj){
	...
	// Option
		...
		if(bd.find('div.rd').length){
			...
			// Content Images
			if(bdImgOpt) bd.find('.x2b_content img').draggable();
			if(bdImgLink){
				bd.find('.x2b_content img').click(function(){
			...
			if(bd.find('form.bd_wrt').length){
				...
				if(bd.find('form>div.wysiwyg').length){
					...
					jQuery.getScript(s_plugin_base_url + "includes/modules/editor/tpl/js/editor_common.min.js",function(){
						editorStartTextarea(2,'content','comment_srl');
						var cmtWrt = bd.find('form.cmt_wrt textarea');
						if(default_style=='blog'){
							cmtWrt.each(function(){
...
function reComment(post_id,cmt_id,edit_url){
	var o = jQuery('#re_cmt').eq(0);
	// o.find('input[name=error_return_url]').val('/'+post_id);
	// o.find('input[name=mid]').val(current_mid);
	o.find('input[name=parent_post_id]').val(post_id);
	o.appendTo(jQuery('#comment_'+cmt_id)).fadeIn().find('input[name=parent_comment_id]').val(cmt_id);
