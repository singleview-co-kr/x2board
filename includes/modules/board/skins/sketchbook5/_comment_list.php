<div id="cmtPosition" aria-live="polite">
	<div class="fdb_tag">
		<a class="ui_font bubble" href="#" onclick="jQuery(this).parent().nextAll('ul,.bd_pg').slideToggle();return false">Comments <b>'<?php echo $post->get_comment_count()?>'</b>
		...
		<?php foreach($post->get_comments() as $key => $comment): // while($comment = $commentList->hasNext()): $commentURL->setCommentUID($comment->uid);?>
			<?php if($comment->get('depth')): ?>
				<li id="comment_<?php echo $comment->comment_id?>" class="fdb_itm clear re bg<?php echo $comment->get('depth')%2?>" style="margin-left:<?php echo (($comment->get('depth')-1)%10+1)*2 ?>%">
					...
					<?php if($comment->get('depth')>10): ?>
					<i class="fa fa-share fa-flip-vertical re rere"></i>
					<?php endif?>
			<?php else:?>
			<li id="comment-uid-<?php echo $comment->uid?>" itemscope itemtype="http://schema.org/Comment" class="fdb_itm clear" data-username="<?php echo $comment->user_display?>" data-created="<?php echo $comment->created?>">
			<?php endif?>
				<span class="profile img no_img">?</span>
				<div class="meta">
					<a href="#popup_menu_area" class="member_4" onclick="return false"><?php echo $comment->get_nick_name()?></a>
				...
				<?php else:?>
					<?php if($comment->remaining_time_for_reading):?>
						<div class="remaining_time_for_reading"><?php echo sprintf(__('You can read comments after %d minutes. <a href="%s">Login</a> and you can read it right away.', 'x2board'), round($comment->remaining_time_for_reading/60), wp_login_url($_SERVER['REQUEST_URI']))?></div>
					<?php elseif($comment->login_is_required_for_reading):?>
						<div class="login_is_required_for_reading"><?php echo sprintf(__('You do not have permission to read this comment. Please <a href="%s">login</a>.', 'x2board'), wp_login_url($_SERVER['REQUEST_URI']))?></div>
					<?php else:?>
				...
				<div class="fdb_nav img_tx">
					...
					<a href="<?php echo x2b_get_url('cmd', X2B_CMD_VIEW_DELETE_COMMENT, 'comment_id',$comment->comment_id)?>" title="<?php echo __('Delete', 'x2board')?>"><i class="fa fa-eraser"></i><?php echo __('Delete', 'x2board')?></a>
					
					<?php if(!$grant->write_comment){
						$s_onclick = "alert('".__('msg_not_permitted', 'x2board')."')";
					}
					else {
						$s_onclick = "reComment({$comment->get('parent_post_id')},{$comment->get('comment_id')},'".x2b_get_url('cmd', X2B_CMD_VIEW_REPLY_COMMENT,'comment_id',$comment->comment_id)."')";
					}
					?>
					<?php if($post->allow_comment()):?>
						<a class="re_comment kboard-reply" href="<?php echo x2b_get_url('cmd', X2B_CMD_VIEW_REPLY_COMMENT,'comment_id',$comment->comment_id) ?>" onclick="<?php echo $s_onclick?>; return false;" title="<?php echo __('Reply', 'x2board')?>"><i class="fa fa-comment"></i> <?php echo __('Reply', 'x2board')?></a>
					<?php endif?>

					<span class="vote ui_font">
						<a class="bd_login" href="#" onclick="kboard_comment_like(this);return false;" data-uid="<?php echo $comment->uid?>" title="cmd_vote"><em><i class="fa fa-thumbs-up"></i> <?php echo intval($comment->like)?></em></a>
						<a class="bd_login" href="#" onclick="kboard_comment_unlike(this);return false;" data-uid="<?php echo $comment->uid?>" title="cmd_vote_down"><i class="fa fa-thumbs-down"></i> <?php echo intval($comment->unlike)?></a>
					</span>		
				</div>
			</li>
			<!-- 답글 리스트 시작 -->
			...
			<!-- 댓글 입력 폼 시작 -->
			<form id="kboard-comment-reply-form-<?php echo $comment->comment_id?>" method="post" action="<?php //echo $commentURL->getInsertURL()?>" class="comments-reply-form" enctype="multipart/form-data" onsubmit="return kboard_comments_execute(this);">
	...
	<div class="bd_pg clear {$mi->fdb_hide}">
		<a href="<?php echo x2b_get_url('cpage', 1)?>#<?php echo $post->get('post_id') ?>_comment" class="direction" title="<?php echo __('first_page', 'x2board')?>"><i class="fa fa-angle-double-left"></i> <span><?php echo __('First', 'x2board')?></span></a>
		...
		<?php endwhile?>
		<!-- </block> -->
		<a href="<?php echo x2b_get_url('cpage', $post->comment_page_navigation->n_last_page)?>#<?php echo $post->get('post_id') ?>_comment" class="direction" title="<?php echo __('last_page', 'x2board')?>"><span><?php echo __('Last', 'x2board')?></span> <i class="fa fa-angle-double-right"></i></a>
	</div>
	<?php endif?>
	<!-- </block> -->
</div>
<?php //wp_enqueue_script('x2board-comments-script', "{$skin_url}/js/comment_script.js", array(), X2B_VERSION, true)?>