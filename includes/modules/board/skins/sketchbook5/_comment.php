...
<?php if($mi->fdb_style=='fdb_v2' && $mi->default_style!='guest'):?><!-- cond="$mi->fdb_style=='fdb_v2' && $mi->default_style!='guest'"  -->
	<a class="nametag ui_font bubble" href="#" onclick="jQuery(this).nextAll('ul,.bd_pg').slideToggle();return false" title="Click! <?php echo __('comment', 'x2board')?> <?php echo __('cmd_view', 'x2board')?>~"><strong class="bg_color bx_shadow">Comment <b>'<?php echo $post->get_comment_count()?>'</b></strong></a>
<?php endif?>
<!--// Comment v1 -->
<?php if($post->get_comment_count()):?><!-- <block cond="$oDocument->getCommentcount()"> -->
	<?php if($mi->fdb_style == ' '):?><!-- cond="!$mi->fdb_style"  -->
		<div class="fdb_tag<?php if($mi->cmt_wrt_position=='cmt_wrt_btm'):?> bg_f_f9 css3pie<?php endif?>">
			<a class="ui_font bubble" href="#" onclick="jQuery(this).parent().nextAll('ul,.bd_pg').slideToggle();return false" title="<?php echo __('comment', 'x2board')?> <?php echo __('cmd_view', 'x2board')?>">Comments <b>'<?php echo $post->get_comment_count()?>'</b></a>
		</div>
	<?php endif?>
			...
			<?php endif?><!--@end-->
					...
					<div class="meta">
						...
						<?php if($comment->has_uploaded_files()):?><!-- <block cond="$comment->has_uploaded_files()"> -->
							<a class="tg_btn2" href="#files_<?php echo $comment->comment_id?>"><b class="ui-icon ui-icon-disk">Files</b><?php echo __('uploaded_file', 'x2board')?> <small>(<?php echo$comment->get('uploaded_count')?>)</small></a>
							<div id="files_<?php echo $comment->comment_id?>" class="cmt_files tg_cnt2">
								<button type="button" class="tg_blur2"></button><button type="button" class="tg_close2" title="<?php echo __('cmd_close', 'x2board')?>"><b class="ui-icon ui-icon-closethick">X</b></button>
								<ul class="wrp">
					...
					<?php if(!$comment->is_accessible()): ?>
						<form action="./" method="get" onsubmit="return procFilter(this, input_password)">
							...
							<p>&quot;<?php echo __('msg_is_secret', 'x2board')?>&quot;</p>
							<span class="itx_wrp">
								<label for="cpw_<?php echo $comment->comment_id?>"><?php echo __('password', 'x2board')?></label>
								<input type="password" id="cpw_<?php echo $comment->comment_id?>" name="password" class="itx" />
								<input type="submit" value="<?php echo __('cmd_input', 'x2board')?>" class="bd_btn" />
							</span>
						</form>
					<?php else: ?>
					...
					<div class="fdb_nav img_tx">
						<?php if($mi->cmt_this_btn=='2' && $is_logged): ?><!-- <block cond="$mi->cmt_this_btn=='2' && $is_logged"> -->
							<a href="#" onclick="doCallModuleAction('comment','procCommentDeclare','<?php echo $comment->comment_id?>');return false"><i class="fa fa-phone"></i><?php echo __('cmd_declare', 'x2board')?></a>
							<a href="#" onclick="doCallModuleAction('comment','procCommentVoteUp','<?php echo $comment->comment_id?>');return false"><i class="fa fa-heart color"></i><?php echo __('cmd_vote', 'x2board')?></a>
							<a cond="$mi->cmt_vote_down == ' '" href="#" onclick="doCallModuleAction('comment','procCommentVoteDown','<?php echo $comment->comment_id?>');return false"><i class="fa fa-heart"></i><?php echo __('cmd_vote_down', 'x2board')?></a>
						<?php endif ?><!-- </block> -->
						<?php if(($mi->cmt_this_btn == ' ' && $is_logged) || $grant->manager): ?><!-- cond="(!$mi->cmt_this_btn && $is_logged) || $grant->manager" -->
							<a class="comment_<?php echo $comment->comment_id?> m_no" href="#popup_menu_area" onclick="return false"><i class="fa fa-ellipsis-h"></i><?php echo __('cmd_comment_do', 'x2board')?></a>
						<?php endif ?>
						<?php if($comment->is_granted() || !$comment->get('comment_author')): ?><!-- <block cond="$comment->isGranted() || !$comment->get('comment_author')"> -->
							<a href="<?php echo x2b_get_url('cmd', X2B_CMD_VIEW_MODIFY_COMMENT, 'comment_id',$comment->comment_id)?>"><i class="fa fa-pencil"></i><?php echo __('cmd_modify', 'x2board')?></a>
							<a href="<?php echo x2b_get_url('cmd', X2B_CMD_VIEW_DELETE_COMMENT, 'comment_id',$comment->comment_id)?>"><i class="fa fa-eraser"></i><?php echo __('cmd_delete', 'x2board')?></a>
						<?php endif ?><!-- </block> -->
						<?php if($post->allow_comment() ): ?><!-- cond="$oDocument->allowComment()"  -->
							<a class="re_comment" href="<?php echo x2b_get_url('cmd', X2B_CMD_VIEW_REPLY_COMMENT,'comment_id',$comment->comment_id)?>" onclick="<?php if(!$grant->write_comment || !$post->is_enable_comment()):?>alert('<?php echo __('msg_not_permitted', 'x2board')?>')<?php else: ?>reComment(<?php echo $comment->get('parent_post_id')?>,<?php echo $comment->get('comment_id')?>,'<?php echo x2b_get_url('cmd', X2B_CMD_VIEW_REPLY_COMMENT,'comment_id',$comment->comment_id)?>')<?php endif ?>;return false;"><i class="fa fa-comment"></i> <?php echo __('cmd_reply', 'x2board')?></a>
						<?php endif ?>
						<!--// 추천-비추천 -->
						<?php if($mi->cmt_vote!='N' && ($mi->cmt_vote=='2' || $comment->get('voted_count')!=0 || $comment->get('blamed_count')!=0) ): ?><!-- cond="$mi->cmt_vote!='N' && ($mi->cmt_vote=='2' || $comment->get('voted_count')!=0 || $comment->get('blamed_count')!=0)" -->
							<span class="vote ui_font">
								<a class="bd_login" href="#" onclick="doCallModuleAction('comment','procCommentVoteUp','<?php echo $comment->comment_id?>');return false"|cond="$is_logged" title="<?php echo __('cmd_vote', 'x2board')?>"><em><i class="fa fa-heart color"></i> <?php echo $comment->get('voted_count') ? $comment->get('voted_count') : 0 ?></em></a>
								<a cond="$mi->cmt_vote_down == ' " class="bd_login" href="#" <?php if($is_logged ): ?> onclick="doCallModuleAction('comment','procCommentVoteDown','<?php echo $comment->comment_id?>');return false" <?php endif ?> title="<?php echo __('cmd_vote_down', 'x2board')?>"><i class="fa fa-heart"></i> <?php echo abs($comment->get('blamed_count') ? $comment->get('blamed_count') : 0)?></a>
							</span>
						<?php endif ?>
					</div>
				</li>
	...
	<?php if($post->comment_page_navigation):?><!-- <block cond="$oDocument->comment_page_navigation"> -->
		<div class="bd_pg clear {$mi->fdb_hide}">
			<a href="<?php echo x2b_get_url('cpage',1)?>#<?php echo $post->get('parent_post_id')?>_comment" class="direction" title="<?php echo __('first_page', 'x2board')?>"><i class="fa fa-angle-double-left"></i> <span>First</span></a>
			<?php while($page_no = $post->comment_page_navigation->getNextPage()): ?><!-- <block loop="$page_no=$oDocument->comment_page_navigation->getNextPage()"> -->
				...
				<?php endif?>
			<?php endwhile?><!-- </block> -->
			<!-- <a href="{getUrl('cpage',$oDocument->comment_page_navigation->last_page)}#{$oDocument->get('document_srl')}_comment" class="direction" title="<?php echo __('last_page', 'x2board')?>"><span>Last</span> <i class="fa fa-angle-double-right"></i></a> -->
			<a href="<?php echo x2b_get_url('cpage', $post->comment_page_navigation->n_last_page)?>#<?php echo $post->get('parent_post_id') ?>_comment" class="direction" title="<?php echo __('last_page', 'x2board')?>"><span>Last</span> <i class="fa fa-angle-double-right"></i></a>
		</div>
	<?php endif ?><!-- </block> -->
<?php endif?><!-- </block> -->